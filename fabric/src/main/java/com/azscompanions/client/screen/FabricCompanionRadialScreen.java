...
/** Fabric pie menu for Follow / Stay / Wander / Emote. */
public final class FabricCompanionRadialScreen extends Screen {
    ...
    public FabricCompanionRadialScreen(int companionId) {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.drawCenteredString(font, page == Page.EMOTE ? "Emote" : "Kon", cx, cy - 4, 0xE8F4FF);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        FabricNetworkingClient.sendRadial(companionId, slice.command().name());
        onClose();
        return true;
    }
    ...
    private Slice[] currentSlices() {
        if (page == Page.EMOTE) {
            return new Slice[]{
                    new Slice(Component.translatable("screen.azscompanions.radial.wave"), FabricRadialCommandMenu.Command.EMOTE_WAVE),
                    new Slice(Component.translatable("screen.azscompanions.radial.cheer"), FabricRadialCommandMenu.Command.EMOTE_CHEER)
            };
        }
        return new Slice[]{
                new Slice(Component.translatable("screen.azscompanions.radial.follow"), FabricRadialCommandMenu.Command.FOLLOW),
                new Slice(Component.translatable("screen.azscompanions.radial.stay"), FabricRadialCommandMenu.Command.STAY),
                new Slice(Component.translatable("screen.azscompanions.radial.wander"), FabricRadialCommandMenu.Command.WANDER),
    ...
    private static int sliceAt(int mouseX, int mouseY, int cx, int cy, int inner, int outer, int count) {
        ...
        angle = (angle + Math.PI * 2.5d) % (Math.PI * 2.0d);
    ...
    public static void openForOwnedCompanion() {
        ...
        var companions = mc.level.getEntitiesOfClass(
                ...
                c -> c.isAlive() && c.isOwnedBy(mc.player));
        ...
        mc.setScreen(new FabricCompanionRadialScreen(companions.getFirst().getId()));
    }

    private record Slice(Component label, FabricRadialCommandMenu.Command command) {
    }
}
