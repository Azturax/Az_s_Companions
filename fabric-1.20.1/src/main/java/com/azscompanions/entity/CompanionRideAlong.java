...
public final class CompanionRideAlong {
    ...
    public static CompanionRideAlongSupport.RideKind classify(@Nullable Entity entity) {
        ...
        // Non-rideable minecart subtypes (furnace/hopper/etc.) — ignore.
    ...
    /**
     * Nearest matching entity for a light approach when no empty mount is available
     * (still never the owner's vehicle).
     */
    @Nullable
    ...
    public static void steerVehicleTowardOwner(Entity vehicle, Player owner, double personalSpace) {
        ...
        double[] vel = CompanionRideAlongSupport.steerVelocity(
                ...
                preferred,
                speed);
        vehicle.setDeltaMovement(vel[0], vehicle instanceof Boat ? vel[1] * 0.35d : vehicle.getDeltaMovement().y * 0.6d + vel[1] * 0.4d, vel[2]);
