...
public final class CompanionPotionHelper {
    ...
    public static void throwPotionAt(LivingEntity thrower, ItemStack potionStack, LivingEntity target) {
        ...
        ItemStack thrownStack = potionStack.copyWithCount(1);
        AbstractThrownPotion thrown = potionStack.is(Items.LINGERING_POTION)
                ? new ThrownLingeringPotion(level, thrower, thrownStack)
                : new ThrownSplashPotion(level, thrower, thrownStack);
            ...
            if (effect.getEffect().value().isInstantenous()) {
                if (beneficiary.level() instanceof net.minecraft.server.level.ServerLevel serverLevel) {
                    effect.getEffect().value().applyInstantenousEffect(
                            serverLevel, null, null, beneficiary, effect.getAmplifier(), 1.0);
                }
            } else {
