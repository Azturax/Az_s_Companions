...
public final class FabricCraftTask extends FabricCompanionTask {
    ...
    public Result tick(FabricCompanionEntity companion, ServerLevel level) {
        ...
        var recipeOpt = level.getServer().getRecipeManager().byKey(
        ...
        ItemStack result = resolveRecipeResult(level, holder).copy();
        if (result.isEmpty()) {
            return Result.FAILED;
        }
        if (!consumeIngredients(companion, holder.value().placementInfo().ingredients())) {
            return Result.FAILED;
        }
