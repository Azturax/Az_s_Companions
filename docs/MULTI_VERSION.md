...
Az's Companions **0.3.7** ships production jars for **Minecraft 1.21.1** only (standalone + CCI for both loaders). Additional Minecraft lines are tracked here for follow-up ports.
...
| `1.21.11` â†’ **1.21.1** | **1.21.1** | **21.1.248** | Fabric API `0.116.15+1.21.1` | **Shipped in 0.3.7** |
...
| Feature parity with 0.3.7 (AI, Behavior, CCI) | Blocked until API port exists |
...
## Shipped 0.3.7 jars (1.21.1 only)

- `azscompanions-neoforge-0.3.7+1.21.1.jar`
- `azscompanions-neoforge-cci-0.3.7+1.21.1.jar` (CCI 1.13.0 `AySbAgcO` + iChunUtil 1.0.3 `OvIyyNh4`)
- `azscompanions-fabric-0.3.7+1.21.1.jar`
- `azscompanions-fabric-cci-0.3.7+1.21.1.jar` (CCI 1.13.0 `PERd6IT9` + iChunUtil 1.0.3 `gfAOoiwe`)

**NeoForge 26.2 jar:** *none yet* (no release tag / artifact). Label when ready: e.g. `0.3.7+neoforge-26.2` / `azscompanions-neoforge-0.3.7+26.2.jar`.
