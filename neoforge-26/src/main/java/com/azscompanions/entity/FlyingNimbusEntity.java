...
/**
 * Rideable Flying Nimbus (Jindujun / Überschallwolke). Player-controlled while mounted —
 * not an AI follower. Soft yellow cloud; trails rendered client-side at cloud height.
 */
public final class FlyingNimbusEntity extends Mob implements PlayerRideable {
    ...
    public static FlyingNimbusEntity createFor(ServerLevel level, Player owner) {
        FlyingNimbusEntity cloud = ModEntities.FLYING_NIMBUS.get().create(level);
        if (cloud == null) {
            return null;
        }
    ...
    public void travel(Vec3 travelVector) {
        if (isAlive() && isVehicle() && getControllingPassenger() instanceof Player player) {
            ...
            // Look pitch contributes mild climb when pressing forward (DBZ-style lean).
