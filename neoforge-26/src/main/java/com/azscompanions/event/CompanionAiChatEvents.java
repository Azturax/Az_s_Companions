...
public final class CompanionAiChatEvents {
    ...
    public static void onServerChat(ServerChatEvent event) {
        ...
        String prompt = ownerLike
                ? CompanionAiChatSupport.chatReactionPrompt(speaker.getGameProfile().name(), full)
                : CompanionAiChatSupport.strangerAddressPrompt(speaker.getGameProfile().name(), full);
        if (CompanionAiAsk.askQuiet(owner, companion, speaker.getGameProfile().name(), prompt,
    ...
    static boolean tryNameMention(ServerPlayer speaker, String rawText, CompanionAiSettings settings) {
        ...
        String prompt = CompanionNameMention.mentionPrompt(
                speaker.getGameProfile().name(), full, ownerLike);
        boolean ok = CompanionAiAsk.askQuiet(owner, companion, speaker.getGameProfile().name(), prompt,
                trust, ownerLike ? null : speaker);
