...
/**
 * Resolves companion skins from resource locations, Mojang {@code player:<uuid>},
 * {@code local:} files, and {@code url:}/{@code http(s):} downloads.
...
public final class CompanionSkinTextures {
    ...
    private static final Map<String, Identifier> EXTERNAL_TEXTURE_CACHE = new ConcurrentHashMap<>();
    ...
    public static Identifier resolve(CompanionEntity entity) {
        if (ClientAppearanceDraft.matches(entity)) {
            ...
            if (draftSkin != null && !draftSkin.isBlank()) {
                Identifier ready = resolveReadyOrNull(draftSkin);
                if (ready != null) {
                    return ready;
                }
        ...
        Identifier ready = resolveReadyOrNull(skinPath);
        if (ready != null) {
            return ready;
        }
    ...
    @Nullable
    public static Identifier resolveReadyOrNull(String skinPath) {
        if (skinPath == null || skinPath.isBlank()) {
            return null;
        }
        ...
        if (CompanionContextSkinSupport.isLocalSkin(skinPath) || CompanionContextSkinSupport.isUrlSkin(skinPath)) {
            ...
            Identifier cached = EXTERNAL_TEXTURE_CACHE.get(key);
            if (cached != null) {
                return cached;
            }
        ...
        CompletableFuture.supplyAsync(() -> loadExternalBytes(skinPath))
                .whenComplete((bytes, err) -> mc.execute(() -> {
                    ...
                    try {
                        ...
                        DynamicTexture texture = new DynamicTexture(() -> "azscompanions/external_skin", image);
