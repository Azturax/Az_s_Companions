...
/** Server-gated Az admin panel (NeoForge 26.2). Profiles fill defaults; all fields stay editable. Ask-only. */
public final class AzAdminScreen extends Screen {
    ...
    private void initAi(int bx, int py, int bw, int panelH) {
        ...
        apiKeyBox.addFormatter((text, offset) -> FormattedCharSequence.forward(
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(font, tab == Tab.OVERVIEW ? "Az Admin" : "AI config → server (Save & apply) · /ask only",
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (tab == Tab.OVERVIEW) {
            ...
            graphics.text(font, truncate(liveApiKeyStatusLabel(), 48),
