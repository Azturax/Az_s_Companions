...
public final class NeoAiJoinOfferEvents {
    ...
    public static void bootstrap() {
        NeoForge.EVENT_BUS.register(NeoAiJoinOfferEvents.class);
    }
    ...
    public static void onLogin(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        PacketDistributor.sendToPlayer(player, CompanionAiJoinOfferPacket.fromOffer(offer));
    }
    ...
    public static void handleConsent(ServerPlayer player, boolean accepted, String suggestProfile, boolean applyProfile) {
        ...
        player.sendSystemMessage(Component.literal(AiJoinConsentApply.TIP));
        ...
        try {
            ...
            if (wantApply) {
                player.sendSystemMessage(Component.literal(
                        "Companion AI enabled with your local LLM (Use server LLM stays OFF — turn it ON in AI Config to share with others)."));
            } else if (merged.serverLlmOnly()) {
                player.sendSystemMessage(Component.literal(
                        "Use server LLM is ON for this host (shared endpoint)."));
            }
        } catch (Exception e) {
            player.sendSystemMessage(Component.literal(
                    "Could not save companion AI settings after consent."));
        }
    }
}
