...
public final class SpecialPlayerPerks {
    ...
    private static void snapBesideOwner(Mob companion, Player owner, double sideOffset, double personalSpace) {
        ...
        if (CompanionSafeTeleportSupport.tooCloseToOwner(offset.x, offset.z, CompanionSafeTeleportSupport.MIN_OWNER_SEPARATION)) {
            double[] behind = CompanionSafeTeleportSupport.behindOwner(owner.getYRot(), offsetDist);
        ...
        companion.invulnerableTime = Math.max(
                companion.invulnerableTime, CompanionSafeTeleportSupport.POST_TELEPORT_INVULN_TICKS);
        companion.getLookControl().setLookAt(owner, 10.0f, companion.getMaxHeadXRot());
    }

    /** Ground snap beside {@code target} — never inside their hitbox. */
    public static void safeTeleportBeside(Mob mob, net.minecraft.world.entity.Entity target, double distance) {
        if (mob == null || target == null) {
            return;
        }
        double dist = Math.max(CompanionSafeTeleportSupport.MIN_OWNER_SEPARATION, distance);
        double[] behind = CompanionSafeTeleportSupport.behindOwner(target.getYRot(), dist);
        mob.teleportTo(target.getX() + behind[0], target.getY(), target.getZ() + behind[1]);
        mob.setDeltaMovement(Vec3.ZERO);
        mob.hasImpulse = true;
        mob.fallDistance = 0.0f;
        mob.invulnerableTime = Math.max(mob.invulnerableTime, CompanionSafeTeleportSupport.POST_TELEPORT_INVULN_TICKS);
    }
