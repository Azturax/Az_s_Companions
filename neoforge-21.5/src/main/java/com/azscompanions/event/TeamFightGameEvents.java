...
public final class TeamFightGameEvents {
    ...
    public static void onLivingDeath(LivingDeathEvent event) {
        ...
        if (OwnableUuids.get(victim) == null || !OwnableUuids.get(victim).equals(OwnableUuids.get(killer))) {
            return;
        }
        ServerPlayer owner = victim.level().getServer() == null
                ? null
                : victim.level().getServer().getPlayerList().getPlayer(OwnableUuids.get(victim));
        if (owner == null) {
            return;
        }
