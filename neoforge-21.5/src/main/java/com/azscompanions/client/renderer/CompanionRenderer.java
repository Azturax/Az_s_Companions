 ...
 * Typed against {@link PlayerRenderState} so vanilla player layers (armor/hands/wings) infer cleanly;
 * companion fields live on {@link CompanionRenderState}.
 */
public final class CompanionRenderer
        extends MobRenderer<CompanionEntity, PlayerRenderState, FeminineCompanionModel> {
    ...
    @Override
    public void extractRenderState(CompanionEntity entity, PlayerRenderState base, float partialTick) {
    ...
    public void render(PlayerRenderState base, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        ...
        if (!state.form.isPlayer()) {
            ...
            CompanionEntity entity = resolveEntity(state);
            if (entity != null) {
    ...
    @Nullable
    private static CompanionEntity resolveEntity(CompanionRenderState state) {
    ...
    @Override
    protected boolean shouldShowName(CompanionEntity entity, double distanceToCameraSq) {
        if (ClientAppearanceDraft.matches(entity)) {
    ...
    protected void renderNameTag(PlayerRenderState state, Component displayName, PoseStack poseStack,
        ...
        CompanionEntity entity = resolveEntity((CompanionRenderState) state);
        if (entity == null) {
    ...
    private static final class CompanionCapeLayer
        ...
        CompanionCapeLayer(CompanionRenderer parent, EntityRendererProvider.Context context) {
            super(parent);
