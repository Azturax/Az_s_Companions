...
/**
 * Soft JourneyMap plugin. Loaded only when JourneyMap is present (annotation discovery).
 * Do not reference this class from mandatory init paths that would classload JM types when absent —
 * {@link com.azscompanions.compat.map.MapCompatModule} only logs the class name when JM is loaded.
 */
@JourneyMapPlugin(apiVersion = "2.0.0")
public final class JourneyMapCompanionPlugin implements IClientPlugin {
    private static final ResourceLocation COMPANION_ICON =
            ResourceLocation.fromNamespaceAndPath(AzsCompanions.MOD_ID, "textures/entity_icon/companion.png");

    @Override
    public String getModId() {
        return AzsCompanions.MOD_ID;
    }

    @Override
    public void initialize(IClientAPI api) {
        ClientEventRegistry.ENTITY_REGISTRATION_EVENT.subscribe(AzsCompanions.MOD_ID, this::onRegisterEntities);
        ClientEventRegistry.ENTITY_RADAR_UPDATE_EVENT.subscribe(AzsCompanions.MOD_ID, this::onRadarUpdate);
        AzsCompanions.LOGGER.info("JourneyMap companion plugin initialized");
    }

    private void onRegisterEntities(EntityRegistrationEvent event) {
        // PathfinderMob companions are already tracked as passive; register explicitly as villager/NPC style.
        event.addVillagerEntity(CompanionEntity.class);
    }
    ...
    private void onRadarUpdate(EntityRadarUpdateEvent event) {
        ...
        if (!CompanionMapEntity.isCompanion(entity) && !(entity instanceof CompanionEntity)) {
            return;
        }
        boolean child = entity instanceof CompanionEntity companion
        ...
        if (settings.showNameOnMap()) {
            String name = entity instanceof CompanionEntity companion
