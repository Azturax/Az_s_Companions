...
public final class CompanionRideAlong {
    ...
    public static void ensureSaddled(Entity candidate) {
        ...
        if (!equippable.canBeEquippedBy(mob.getType())) {
            return;
        }
        mob.setItemSlot(EquipmentSlot.SADDLE, saddle);
        mob.level().playSound(null, mob.getX(), mob.getY(), mob.getZ(),
                equippable.equipSound().value(), SoundSource.NEUTRAL, 0.5f, 1.0f);
    }
    ...
    private static boolean isOwnedByOther(Entity entity, UUID ownerId) {
        ...
        UUID uuid = OwnableUuids.get(ownable);
