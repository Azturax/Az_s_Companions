...
/** NBT helpers for Companion Charm bind / stored companion payload. */
public final class CharmData {
    ...
    public static UUID getBoundUuid(ItemStack stack) {
        CompoundTag tag = getTag(stack);
        return NbtUuids.has(tag, TAG_BOUND) ? NbtUuids.get(tag, TAG_BOUND) : null;
    }

    public static void bind(ItemStack stack, UUID companionUuid) {
        CompoundTag tag = getTag(stack);
        NbtUuids.put(tag, TAG_BOUND, companionUuid);
        setTag(stack, tag);
    }
    ...
    public static CompoundTag peekStoredCompanion(ItemStack stack) {
        ...
        return tag.contains(TAG_STORED) ? tag.getCompoundOrEmpty(TAG_STORED) : null;
    }
