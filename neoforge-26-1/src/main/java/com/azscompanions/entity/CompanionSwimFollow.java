 ...
 * <p>
 * NeoForge 26.2: uses {@code hurtMarked} (replaces {@code hasImpulse}) and {@code isInWater()}
 * (bubble columns count as water; {@code isInWaterOrBubble} was removed).
 */
public final class CompanionSwimFollow {
    ...
    public static boolean isOwnerInWater(Player owner) {
        return owner != null
                && (owner.isSwimming() || owner.isUnderWater() || owner.isInWater());
    }

    public static boolean isCompanionInWater(Mob companion) {
        return companion.isUnderWater() || companion.isInWater();
    }
