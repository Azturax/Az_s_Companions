...
import com.azscompanions.client.renderer.CompanionRenderState;
 ...
 * Typed against {@link AvatarRenderState} for NeoForge 26.2.
 */
public final class FeminineCompanionModel extends PlayerModel {
    ...
    public void setupAnim(AvatarRenderState state) {
        resetPartScales();
        if (state instanceof CompanionRenderState companion && companion.passengerSitPose) {
            state.isPassenger = true;
        }
        super.setupAnim(state);
        boolean showBust = true;
        if (state instanceof CompanionRenderState companion) {
            showBust = companion.showBust;
            applyProportions(
                    companion.bust,
                    companion.waist,
                    companion.hips,
                    companion.shoulders,
                    companion.bustOffset,
                    showBust);
        } else {
            applyProportions(
