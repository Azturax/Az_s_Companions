...
public final class CompanionStatsScreen extends Screen {
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        // Keep the world sharp behind the panel (no menu blur).
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(font, title, panelX + panelW / 2, panelY + 8, TITLE_COLOR);
        ...
        graphics.text(font, Component.translatable("screen.azscompanions.stats.section.companion"),
                leftX, y, SECTION_COLOR, false);
        graphics.text(font, Component.translatable("screen.azscompanions.stats.section.owner"),
        ...
        row = line(graphics, leftX, row, colW, "screen.azscompanions.stats.mode", companion.getMode().getSerializedName());
        ...
        row = line(graphics, leftX, row, colW, "screen.azscompanions.stats.children",
                childCount + "/" + com.azscompanions.config.ServerConfig.MAX_CHILD_COMPANIONS_PER_LEADER.get());
        ...
        String playerName = player != null ? player.getGameProfile().name() : "—";
    ...
    private int line(GuiGraphicsExtractor graphics, int x, int y, int maxW, String labelKey, String value) {
        String label = Component.translatable(labelKey).getString();
        String text = label + ": " + (value == null ? "—" : value);
        String clipped = font.plainSubstrByWidth(text, maxW);
        graphics.text(font, clipped, x, y, VALUE_COLOR, false);
        return y + 11;
    }
